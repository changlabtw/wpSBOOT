...
sub concatenate_output
{
	...
	if(!defined $output)
	{
	  $out = Bio::AlignIO->new(-fh => \*STDOUT , '-format' => 'phylip');
	}
	else
	{
	  $out = Bio::AlignIO->new(-file => ">$output" , '-format' => 'phylip');
	}
...
{
	#no input options
	if((!@ARGV) or	!GetOptions('help|?'=>\$help,'intersect'=>\$intersect,'random'=>\$random,'size=i'=>\$size,'replicate=i'=>\$replicate,'aln=s{1,20}'=>\@files,'out=s'=>\$output)
	or defined $help 
